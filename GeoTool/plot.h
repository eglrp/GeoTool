...
int OCplot(int Cnt,double xishu[4],double xsample[],double ysample[],int size,string name){

	double *X = new double[Cnt];
	double *Y = new double[Cnt];
	CvScalar color_black = CV_RGB(0, 0, 0);

	//	cvNamedWindow("像元畸变",1);
	Plot plot;
//	plot.xlabel("Pixel Position",color_black);
//	plot.ylabel("Nums of Distortion",color_black);
	CvScalar color = CV_RGB(255, 0, 0);
	CvScalar color1 = CV_RGB(0, 0, 255);

	for(int i = 0; i < Cnt; i++)
	{
		X[i] = (double)i;
		Y[i] = (xishu[3]*pow(i,3)+xishu[2]*pow(i,2)+xishu[1]*i+xishu[0]);
	}
	plot.plot(X, Y, Cnt, color, '.');
	cvShowImage(name.c_str(), plot.Figure);
	//	cvWaitKey(0);

	plot.plot(xsample, ysample, size, color1, '+');
	cvShowImage(name.c_str(), plot.Figure);
	cvWaitKey(0);

	cvSaveImage((name+".jpg").c_str(), plot.Figure);
